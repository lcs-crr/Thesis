...
class OmniAnomaly_Encoder(tf.keras.Model):
    ...
    def build_encoder(self):
        ...
        x = tfkl.GRU(
            ...
            activity_regularizer=tf.keras.regularizers.L2(l2=1e-4),
        )(enc_input)
        ...
        z_mean = tfkl.TimeDistributed(
            tfkl.Dense(
                self.latent_dim, activity_regularizer=tf.keras.regularizers.L2(l2=1e-4)
            )
        )(h)
        z_logvar = tfkl.TimeDistributed(
            tfkl.Dense(
                self.latent_dim, activity_regularizer=tf.keras.regularizers.L2(l2=1e-4)
            )
        )(h)
        ...
        for k in range(20):
            z = z + tfkl.Dense(self.latent_dim, use_bias=False)(
                tfkl.Dense(self.latent_dim, activation="tanh")(z)
            )
...
class OmniAnomaly_Decoder(tf.keras.Model):
    ...
    def build_decoder(self):
        ...
        xhat_mean = tfkl.TimeDistributed(
            tfkl.Dense(
                self.features, activity_regularizer=tf.keras.regularizers.L2(l2=1e-4)
            )
        )(h)
        xhat_logvar = tfkl.TimeDistributed(
            tfkl.Dense(
                self.features, activity_regularizer=tf.keras.regularizers.L2(l2=1e-4)
            )
        )(h)
