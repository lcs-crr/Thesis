...
class Inferencer(base_class.BaseProcessor):
    ...
    def _reverse_window_array(
        ...
        assert isinstance(input_windows, np.ndarray), (
            "input_windows must be a numpy array!"
        )
        assert input_windows.ndim == 3, (
            "input_windows must be a 3D numpy array of shape (num_windows, window_size, channels)!"
        )
    ...
    def _first_reverse_window_array(
        ...
        output_size = (
            num_windows - 1
        ) * self.window_shift + window_size  # Compute final output size
        # Pre-allocate output array
        data = np.zeros((output_size, num_channels))
        # Assign the first value of each window
        for i in range(num_windows - 1):
            data[i * self.window_shift : (i + 1) * self.window_shift] = input_windows[
                i, : self.window_shift
            ]
    ...
    def _last_reverse_window_array(
        ...
        # Assign the last value of each window
        for i in range(1, num_windows):
            data[
                (i - 1) * self.window_shift + window_size : i * self.window_shift
                + window_size
            ] = input_windows[i, -self.window_shift :]
        return data
    ...
    def _load_keras_model(
        ...
        if model_name == "tevae":
            from model_garden import tevae

            model = tf.keras.models.load_model(
                os.path.join(self.model_path, "model.keras"),
                custom_objects={
            ...
            from model_garden import tcnae
            ...
            from model_garden import sisvae
            ...
            from model_garden import omnianomaly
            ...
            from model_garden import lwvae
            ...
            from model_garden import vsvae
        ...
        elif model_name == "tevaemm":
            from model_garden import tevae_mm

            model = tf.keras.models.load_model(
                os.path.join(self.model_path, "model.keras"),
                custom_objects={
                    "TeVAE_Encoder": tevae_mm.TeVAE_Encoder,
                    "TeVAE_Decoder": tevae_mm.TeVAE_Decoder,
                    "MA": tevae_mm.MA,
                    "TeVAE": tevae_mm.TeVAE,
                },
            )
        else:
    ...
    def inference_list(
        ...
        subset_name: str,
        save_inference_results: bool | None = True,
        ...
        _inference_array = {
            ...
            "tevaemm": self._tevaemm_inference,
        }
        ...
        if save_inference_results:
            if self.reverse_mode == "mean":
                self.dump_pickle(
                    detection_score_list,
                    os.path.join(
                        self.model_path, subset_name + "_detection_score" + ".pkl"
                    ),
                )
                self.dump_pickle(
                    output_list,
                    os.path.join(self.model_path, subset_name + "_output" + ".pkl"),
                )
            else:
                self.dump_pickle(
                    detection_score_list,
                    os.path.join(
                        self.model_path,
                        subset_name + "_detection_score_" + self.reverse_mode + ".pkl",
                    ),
                )
                self.dump_pickle(
                    output_list,
                    os.path.join(
                        self.model_path,
                        subset_name + "_output_" + self.reverse_mode + ".pkl",
                    ),
                )
    ...
    def _tevae_inference(
        ...
        assert input_array.ndim == 2, (
            "input_array must be a 2D numpy array (time_steps, channels)!"
        )
        assert self.window_size is not None, "window_size must be provided!"
        assert self.window_shift is not None, "window_shift must be provided!"

        # Window input array
        input_windows = self.window_array(
            input_array, self.window_size, self.window_shift
        )
        # Predict output windows
        xhat_mean, xhat_logvar, xhat, _, _, _, _ = model.predict(
            input_windows, batch_size=self.batch_size, verbose=0
        )
        ...
        anomaly_score = model.rec_fn(
            input_array, [xhat_mean, np.log(xhat_var)], reduce_time=False
        ).numpy()
    ...
    def _sisvae_inference(
        ...
        # Predict output windows
        xhat_mean, xhat_logvar, xhat, _, _, _ = model.predict(
            input_windows, batch_size=self.batch_size, verbose=0
        )
        # Calculate variance parameter
        xhat_var = np.exp(xhat_logvar)
    ...
    def _lwvae_inference(
        ...
        (
            ...
            _,
            _,
        ) = model.predict(input_windows, batch_size=self.batch_size, verbose=0)
        # Reverse window reconstruction
        xhat = self._reverse_window_array(xhat)
        # Calculate negative log likelihood
        anomaly_score = model.rec_fn(input_array, xhat, reduce_time=False).numpy()
    ...
    def _tevaemm_inference(
        ...
        # Calculate negative log likelihood
        anomaly_score = model.rec_fn(
            input_array[:, : -model.modal_features],
            [xhat_mean, np.log(xhat_var)],
            reduce_time=False,
        ).numpy()
        # Clear GPU memory before next call
        tf.keras.backend.clear_session()
