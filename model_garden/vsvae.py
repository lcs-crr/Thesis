...
class VSVAE(tf.keras.Model):
    ...
    def rec_fn(x, xhat_params, reduce_time=True):
        ...
        output_dist = tfd.Laplace(
            loc=xhat_mean, scale=tf.sqrt(tf.math.exp(xhat_logvar))
        )
    ...
    def att_fn(att_params, reduce_time=True):
        ...
        # Configure distribution with latent parameters
        latent_dist = tfd.MultivariateNormalDiag(
            loc=a_mean, scale_diag=tf.sqrt(tf.math.exp(a_logvar))
        )
        # Calculate KL-Divergence between latent distribution and standard Gaussian
        att_loss = latent_dist.kl_divergence(
            tfd.MultivariateNormalDiag(
                loc=tf.zeros_like(a_mean), scale_diag=tf.ones_like(a_logvar)
            )
        )
        if reduce_time:
...
class VS(tf.keras.Model):
    def __init__(
        self, seq_len: int, latent_dim: int, features: int, seed: int, name: str = ""
    ):
    ...
    def build_VS(self):
        ...
        s_det = tf.divide(
            tf.matmul(vs_input, vs_input, transpose_b=True),
