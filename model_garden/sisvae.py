...
class SISVAE(tf.keras.Model):
    def __init__(self, encoder, decoder, name: str = "", **kwargs) -> None:
    ...
    def smooth_fn(z_params, reduce_time=True):
        ...
        smooth_loss = [
            tfd.MultivariateNormalDiag(
                loc=z_mean[:, time_step - 1],
                scale_diag=tf.sqrt(tf.math.exp(z_logvar[:, time_step - 1])),
            ).kl_divergence(
                tfd.MultivariateNormalDiag(
                    loc=z_mean[:, time_step],
                    scale_diag=tf.sqrt(tf.math.exp(z_logvar[:, time_step])),
                ),
            )
