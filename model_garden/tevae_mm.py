...
class TeVAE(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.beta = tf.Variable(
            beta, trainable=False
        )  # Weight for KL-Loss, can be modified with a callback
    ...
    def rec_fn(x, xhat_params, reduce_time=True):
        ...
        output_dist = tfd.MultivariateNormalDiag(
            loc=xhat_mean, scale_diag=tf.sqrt(tf.math.exp(xhat_logvar))
        )
    ...
    def train_step(self, x):
        x_static = x[:, :, -self.modal_features :]
        x_dynamic = x[:, :, : -self.modal_features]
        with tf.GradientTape() as tape:
            # Forward pass through encoder
            z_mean, z_logvar, z = self.encoder([x_dynamic, x_static], training=True)
            # Forward pass through MA
            c = self.ma([x_dynamic, z], training=True)
            # Forward pass through decoder
            xhat_mean, xhat_logvar, xhat = self.decoder([c, x_static], training=True)
            # Calculate losses from parameters
            rec_loss = self.rec_fn(x_dynamic, [xhat_mean, xhat_logvar])
    ...
    def test_step(self, x):
        ...
        x_dynamic = x[:, :, : -self.modal_features]
        # Forward pass through encoder
        z_mean, z_logvar, z = self.encoder([x_dynamic, x_static], training=False)
        # Forward pass through MA
        c = self.ma([x_dynamic, z_mean], training=False)
        # Forward pass through decoder
        xhat_mean, xhat_logvar, xhat = self.decoder([c, x_static], training=False)
        # Calculate losses from parameters
        rec_loss = self.rec_fn(x_dynamic, [xhat_mean, xhat_logvar])
        kl_loss = self.kldiv_fn([z_mean, z_logvar])
        loss = rec_loss + self.beta * kl_loss
        ...
        config.update(
            {
                ...
                "modal_features": int(self.modal_features),
    ...
    def from_config(cls, config, **kwargs):
        ...
        return cls(
            ...
            decoder=decoder,
            ma=ma,
            modal_features=config["modal_features"],
            beta=config["beta"],
        )
...
class TeVAE_Encoder(tf.keras.Model):
    ...
    def build_encoder(self):
        dynamic_input = tfkl.Input(shape=(self.seq_len, self.features))
        noise_dynamic_input = tfkl.GaussianNoise(0.01, seed=self.seed)(dynamic_input)
        static_input = tfkl.Input(shape=(self.seq_len, self.modal_features))
        comb_input = tfkl.Concatenate()([noise_dynamic_input, static_input])
        bilstm = tfkl.Bidirectional(
            tfkl.LSTM(self.hidden_units * 2, return_sequences=True)
        )(comb_input)
        bilstm = tfkl.Bidirectional(
            tfkl.LSTM(self.hidden_units, return_sequences=True)
        )(bilstm)
        ...
        output_dist = tfd.Normal(loc=0.0, scale=1.0)
        eps = output_dist.sample(tf.shape(z_mean), seed=self.seed)
        z = z_mean + tf.sqrt(tf.math.exp(z_logvar)) * eps
        return tf.keras.Model([dynamic_input, static_input], [z_mean, z_logvar, z])
        ...
        config.update(
            {
                ...
                "modal_features": self.modal_features,
...
class TeVAE_Decoder(tf.keras.Model):
    ...
    def build_decoder(self):
        dynamic_input = tfkl.Input(shape=(self.seq_len, self.latent_dim))
        static_input = tfkl.Input(shape=(self.seq_len, self.modal_features))
        comb_input = tfkl.Concatenate()([dynamic_input, static_input])
        bilstm = tfkl.Bidirectional(
            tfkl.LSTM(self.hidden_units, return_sequences=True)
        )(comb_input)
        bilstm = tfkl.Bidirectional(
        ...
        return tf.keras.Model(
            [dynamic_input, static_input], [xhat_mean, xhat_logvar, xhat]
        )
